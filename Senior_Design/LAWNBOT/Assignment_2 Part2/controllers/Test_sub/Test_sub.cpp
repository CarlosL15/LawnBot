...
int modeSelection(Keyboard *keyboard, Robot *robot) {
  cout << "Press V for Manual Mode or N for Automatic Mode or B for Idle Mode\n";
  ...
  while (selectedMode == -1) {
      ...
      switch (key) {
          ...
          case 'B':
            selectedMode = 2; //Idle mode
             cout << "Idle mode selected.\n";
             break;
          default:
              if (key != -1) {
 ...
 */
  ...
  while (robot->step(TIME_STEP) != -1) {
    ...
    */
    ...
    if (mode == 0){
      //Move this upwards, then read here!
      ...
      while (getline(MyFile,line)){
        ...
        if (counter == 1){ //Iterating in reverse
          //while (getline(MyFile, line)){ //Getting one line, at a time, into the line variable 
          //data_lines.push_back(line); //Complicated way to say append
          //}
          for(int i = data_lines.size() - 1; i>= 0; i--){ //Since we are pushing back, we need to 
            stringstream ss(data_lines[i]); //load the string into a stream
            
            while(ss >> word){ //Reads words one by one
              data_words.push_back(word); //append
            }
            double r_x = std::stod(data_words[data_words.size() - 5]); //Applied when using vectors (into doubles)
            double r_y = std::stod(data_words[data_words.size() - 4]); //Same 
            double r_z = std::stod(data_words[data_words.size() - 3]); //Same
            char r_input = data_words[data_words.size() - 2][0];  // Convert string to char
            int r_sec = std::stoi(data_words[data_words.size() - 1]); //Same but with ints
            
            while(r_sec > 0){
              cout << "Distance sensor left:" << ds_left_val << endl;
              cout << "Distance sensor right:" << ds_right_val << endl;
        
              cout << r_x << " " << r_y << " " << r_z << " " << r_input << " " << r_sec << endl; //NEVER FORGET endl WHEN ITERATING FILES
              switch(r_input){
                case 'F':
        ...
        break;
          ...
          //  data_lines.push_back(line); //Complicated way to say append
          //}
          for(int i = 0; i <= data_lines.size() - 1; i++){ //Since we are pushing back, we need to 
            stringstream ss(data_lines[i]); //load the string into a stream
      ...
      if(mode == 1){
        ...
        }else if (ds_right_val >= 560 && ds_left_val >= 560){
          leftspeed = -MAX_SPEED/3;
          rightspeed = -MAX_SPEED/3;
